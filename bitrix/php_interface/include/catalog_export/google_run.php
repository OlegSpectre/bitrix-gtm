	...
	if (isset($arProperties[$PROPERTY]) && !empty($arProperties[$PROPERTY]))
	{
		...
		if ($bParam)
		{
				...
				if (!empty($value))
				{
					foreach ($value as $key => $val)
					{
						if ($val != '')
						{
							...
							if (yandex_text2xml($description[$key], true) == 'Цвет'){
			...
			else
			{
				if ($value != '')
				{
					if (yandex_text2xml($iblockProperty['NAME'], true) == 'Цвет'){
						$strProperty .= '<g:color>'.
							yandex_text2xml($value, true).'</g:color>';
					}
					if (yandex_text2xml($iblockProperty['NAME'], true) == 'Размер'){
						$strProperty .= '<g:size>'.
							yandex_text2xml($value, true).'</g:size>';
					}
				}
			}
		}
...
if ($firstStep)
{
	...
	if (empty($arRunErrors))
	{
		...
		fwrite($fp, 'echo "<"."?xml version=\"1.0\"?".">"?>'."\n");
		/*<rss xmlns:g="http://base.google.com/ns/1.0" version="2.0">*/
		fwrite($fp, "\n".'<rss xmlns:g="http://base.google.com/ns/1.0" version="2.0">'."\n");
		fwrite($fp, '<channel>'."\n");
		fwrite($fp, '<title>'.htmlspecialcharsbx($site['SITE_NAME'])."</title>\n");
		fwrite($fp, '<link>'.$usedProtocol.htmlspecialcharsbx($site['SERVER_NAME'])."</link>\n");
		fwrite($fp, '<description>'.htmlspecialcharsbx($site['COMPANY_NAME'])."</description>\n");
			...
			foreach ($itemIdsList as $id)
			{
				...
				if ($row['TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					...
					foreach ($row['OFFERS'] as $offer)
					{
						...
						$itemsContent .= "<g:title>".yandex_text2xml($offer['NAME'], true)."</g:title>\n";
						
						$itemsContent .= "<g:description>".
							($offer['DESCRIPTION'] !== '' ? $offer['DESCRIPTION'] : $row['DESCRIPTION']).
							"</g:description>\n";
						
						$itemsContent .= "<g:link>".$usedProtocol.$site['SERVER_NAME'].htmlspecialcharsbx($offer['DETAIL_PAGE_URL']).$referer."</g:link>\n";
						...
						$itemsContent .= "<g:availability>in stock</g:availability>\n";
						
						$minPrice = $offer['RESULT_PRICE']['MIN_PRICE'];
						$fullPrice = $offer['RESULT_PRICE']['FULL_PRICE'];
						$RUR = 'RUB';
						$itemsContent .= "<g:price>".$minPrice." ".$RUR."</g:price>\n";
						
						$itemsContent .= "<g:brand>INSPIRE</g:brand>\n";
						$itemsContent .= "<g:gender>Female</g:gender>\n";
